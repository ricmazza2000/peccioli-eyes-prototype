...
COLOR_EMOJI = {
    ...
    "#0a5a7a": "🟦",  # Petrolio
    "#b8860b": "🟧",  # Ambra
    "#636363": "⚫",  # Grigio (usa nero piccolo)
    "#4db3e8": "💧",  # Azzurro
    "#c26a8a": "🩷",  # Rosa antico
    "#1a1a1a": "⬛",  # Nero
    "#ffffff": "⬜",  # Bianco
...
def render_swatch_row(state_key, current_value, key_prefix):
    ...
    Usa emoji colorate per distinguere i colori visivamente."""
    cols = st.columns(6)
    for i, (cname, chex) in enumerate(COLORS):
        with cols[i % 6]:
            is_active = (current_value.lower() == chex.lower())
            emoji = COLOR_EMOJI.get(chex, "⚪")
            label = f"{emoji}\n{cname[:6]}"
            btn_type = "primary" if is_active else "secondary"
            if st.button(label, key=f"sw_{key_prefix}_{i}", use_container_width=True, help=cname, type=btn_type):
...
SECURITY_QUESTIONS = [
    ...
    "Come si chiama il tuo migliore amico/a?",
    "Qual è il tuo film preferito?",
    "Qual è il nome del tuo animale (vero o sognato)?",
]
...
def validate_username(u):
    ...
    if not u:
        return False, "Inserisci uno username"
    if len(u) < 3:
        return False, "Lo username deve avere almeno 3 caratteri"
    if len(u) > 20:
        return False, "Lo username deve avere al massimo 20 caratteri"
    if not USERNAME_REGEX.match(u):
        return False, "Lo username può contenere solo lettere, numeri, punti, trattini e underscore (no spazi né simboli)"
    return True, ""

def validate_password(p):
    if not p:
        return False, "Inserisci una password"
    if len(p) < 4:
        return False, "La password deve avere almeno 4 caratteri"
    return True, ""

# Auto-login se username è in session_state (da remember-me)
if st.session_state.username is None and st.session_state.get("remembered_username"):
    remembered = st.session_state.remembered_username
    if remembered in db:
...
if st.session_state.username is None:
    ...
    tab_login, tab_new, tab_forgot = st.tabs(["🔓 Accedi", "✨ Crea profilo", "🔑 Password dimenticata"])

    # ---- ACCEDI ----
    with tab_login:
        c1, c2 = st.columns(2)
        with c1:
            login_user = st.text_input("Username", key="login_user", placeholder="iltuonome")
        with c2:
            login_pwd = st.text_input("Password", type="password", key="login_pwd", placeholder="••••••")
        
        remember = st.checkbox("Resta connesso su questo dispositivo", value=True, key="remember_me")
        
        if st.button("🔓 Entra", key="btn_login", use_container_width=True, type="primary"):
            u = (login_user or "").strip().lower()
            if not u:
                st.error("Inserisci il tuo username")
            elif u not in db:
                st.error(f"Non esiste nessun profilo con username '{login_user}'. Vai su 'Crea profilo' per registrarti.")
    ...
    with tab_new:
        ...
        with c1:
            new_user = st.text_input(
                ...
                help="Solo lettere, numeri, punti, trattini e underscore. No spazi."
            )
        with c2:
            new_display = st.text_input(
                "Nome visibile (nella galleria)",
                key="new_display",
                placeholder="es. Marco R.",
                help="Questo è il nome che vedranno gli altri ragazzi nella galleria"
            )
        
        new_pwd = st.text_input("Inventa una password", type="password", key="new_pwd", placeholder="Almeno 4 caratteri")
        
        st.markdown("##### 🔒 Domanda di sicurezza")
        st.caption("Servirà per recuperare la password se la dimentichi")
        ...
        new_answer = st.text_input(
            "La tua risposta",
            key="new_answer",
            placeholder="Es. 2005, Storia, Luca...",
            help="Sarà confrontata senza distinguere maiuscole/minuscole. Niente spazi iniziali/finali."
        )
        
        if st.button("✨ Crea il mio profilo", key="btn_new", use_container_width=True, type="primary"):
            u = (new_user or "").strip().lower()
            display = (new_display or new_user or "").strip()
            
            # Validazione username
            ok_u, msg_u = validate_username(u)
            ok_p, msg_p = validate_password(new_pwd)
            
            if not ok_u:
                st.error(msg_u)
            elif u in db:
                st.error(f"Lo username '{u}' è già preso. Provane un altro.")
            elif not display or len(display) < 2:
                st.error("Il nome visibile deve avere almeno 2 caratteri")
            elif not ok_p:
                st.error(msg_p)
            elif not new_answer or len(new_answer.strip()) < 1:
                st.error("Inserisci una risposta alla domanda di sicurezza")
            else:
                db[u] = {
                    "display_name": display,
                    "password": new_pwd,
                    "security_question": new_question,
                    "security_answer": new_answer.strip().lower(),
                ...
                for k in list(st.session_state.keys()):
                    if k.startswith("w_") or k.startswith("__"):
                        del st.session_state[k]
                st.success("Profilo creato! Benvenuto/a 🎉")
                st.rerun()

    # ---- PASSWORD DIMENTICATA ----
    with tab_forgot:
        st.caption("Inserisci il tuo username, rispondi alla domanda di sicurezza, e potrai impostare una nuova password.")
        
        forgot_user = st.text_input("Username", key="forgot_user", placeholder="iltuonome")
        
        # Step 1: cerca utente e mostra domanda
        if forgot_user:
            fu = forgot_user.strip().lower()
            if fu in db:
                user_data = db[fu]
                if "security_question" not in user_data:
                    st.warning("Questo profilo è stato creato prima dell'introduzione della domanda di sicurezza. Contatta l'amministratore per il reset.")
                else:
                    st.markdown(f"**Domanda:** {user_data['security_question']}")
                    forgot_answer = st.text_input("La tua risposta", key="forgot_answer")
                    forgot_new_pwd = st.text_input("Nuova password", type="password", key="forgot_new_pwd", placeholder="Almeno 4 caratteri")
                    forgot_new_pwd2 = st.text_input("Conferma nuova password", type="password", key="forgot_new_pwd2")
                    
                    if st.button("🔑 Reimposta password", key="btn_forgot_reset", use_container_width=True, type="primary"):
                        if not forgot_answer or forgot_answer.strip().lower() != user_data.get("security_answer", ""):
                            st.error("Risposta sbagliata. Riprova.")
                        else:
                            ok_p, msg_p = validate_password(forgot_new_pwd)
                            if not ok_p:
                                st.error(msg_p)
                            elif forgot_new_pwd != forgot_new_pwd2:
                                st.error("Le due password non coincidono")
                            else:
                                db[fu]["password"] = forgot_new_pwd
                                save_db(db)
                                st.success("Password aggiornata! Ora puoi accedere dalla scheda 'Accedi'.")
            elif fu:
                st.warning(f"Nessun profilo trovato con username '{fu}'")
...
with col_nav:
    ...
    with nav_cols[2]:
        if st.button("Galleria", key="nav_gallery", use_container_width=True):
            ...
            st.rerun()
    with nav_cols[3]:
        if st.button("Profilo", key="nav_profile", use_container_width=True):
...
# ==================== PROFILO ====================
    ...
    st.markdown('<div class="section-title-home">Il mio profilo</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub-home">Gestisci il tuo account</div>', unsafe_allow_html=True)

    c_left, c_right = st.columns([1, 1.4])

    with c_left:
        # Mostra avatar attuale
        big_svg = eye_svg(avatar, size=240)
        profile_card = f'<div style="background:{BRAND_BLUE_LIGHT};border-radius:22px;padding:1.5rem;text-align:center;">{big_svg}<div style="margin-top:0.8rem;font-family:\'Playfair Display\',Georgia,serif;font-weight:800;font-size:1.3rem;color:{BRAND_BLUE};">{display_name}</div><div style="font-size:0.78rem;color:#5b6472;margin-top:0.2rem;">@{st.session_state.username}</div></div>'
        st.markdown(profile_card, unsafe_allow_html=True)

        st.caption(f"📅 Profilo creato il {user.get('created_at', 'data ignota')[:10]}")
        if user.get("visible_in_gallery", True):
            st.success("✓ Visibile nella galleria")
        else:
            st.info("⊘ Nascosto dalla galleria")

    with c_right:
        # ---- INFO BASE ----
        st.markdown("##### 👤 Informazioni profilo")
        
        new_display_name = st.text_input(
            "Nome visibile (nella galleria)",
            value=display_name,
            key="prof_display",
            help="Questo è il nome che vedono gli altri"
        )
        
        new_visible = st.checkbox(
            "Mostra il mio sguardo nella galleria pubblica",
            value=user.get("visible_in_gallery", True),
            key="prof_visible"
        )
        
        if st.button("💾 Salva modifiche profilo", key="btn_save_profile", use_container_width=True, type="primary"):
            display_clean = (new_display_name or "").strip()
            if len(display_clean) < 2:
                st.error("Il nome visibile deve avere almeno 2 caratteri")
            else:
                db[st.session_state.username]["display_name"] = display_clean
                db[st.session_state.username]["visible_in_gallery"] = new_visible
                save_db(db)
        ...
        with st.expander("Modifica la mia password"):
            old_pwd = st.text_input("Password attuale", type="password", key="prof_old_pwd")
            new_pwd_1 = st.text_input("Nuova password", type="password", key="prof_new_pwd")
            new_pwd_2 = st.text_input("Conferma nuova password", type="password", key="prof_new_pwd2")
            
            if st.button("🔑 Aggiorna password", key="btn_change_pwd", use_container_width=True):
                if old_pwd != user.get("password"):
                    st.error("Password attuale sbagliata")
                else:
                    ok_p, msg_p = validate_password(new_pwd_1)
                    if not ok_p:
                        st.error(msg_p)
                    elif new_pwd_1 != new_pwd_2:
                        st.error("Le due password non coincidono")
                    else:
                        db[st.session_state.username]["password"] = new_pwd_1
                        save_db(db)
        ...
        if "security_question" in user:
            with st.expander("Modifica domanda di sicurezza"):
                st.caption(f"**Domanda attuale:** {user['security_question']}")
                
                new_question = st.selectbox(
                    "Nuova domanda",
                    SECURITY_QUESTIONS,
                    index=SECURITY_QUESTIONS.index(user["security_question"]) if user["security_question"] in SECURITY_QUESTIONS else 0,
                    key="prof_new_q"
                )
                new_answer = st.text_input("Nuova risposta", key="prof_new_a")
                
                if st.button("Aggiorna domanda", key="btn_change_q", use_container_width=True):
                    if not new_answer or len(new_answer.strip()) < 1:
                        st.error("Inserisci una risposta")
                    else:
                        db[st.session_state.username]["security_question"] = new_question
                        db[st.session_state.username]["security_answer"] = new_answer.strip().lower()
                        save_db(db)
                        st.success("Domanda di sicurezza aggiornata!")
        else:
            with st.expander("Aggiungi una domanda di sicurezza"):
                st.info("Questo profilo non ha ancora una domanda di sicurezza. Aggiungerne una ti permette di recuperare la password se la dimentichi.")
                
                new_question = st.selectbox("Scegli una domanda", SECURITY_QUESTIONS, key="prof_add_q")
                new_answer = st.text_input("La tua risposta", key="prof_add_a")
                
                if st.button("Aggiungi domanda", key="btn_add_q", use_container_width=True, type="primary"):
                    if not new_answer or len(new_answer.strip()) < 1:
                        st.error("Inserisci una risposta")
                    else:
                        db[st.session_state.username]["security_question"] = new_question
                        db[st.session_state.username]["security_answer"] = new_answer.strip().lower()
                        save_db(db)
                        st.success("Domanda di sicurezza salvata!")
                        st.rerun()
        ...
        for i in range(0, len(visible_users), cols_per_row):
            ...
            for col, (uname, u) in zip(cols, row):
                with col:
                    ...
                    st.markdown(card, unsafe_allow_html=True)    {"id": "book", "label": "Libro (identita)", "category": "temi"},
